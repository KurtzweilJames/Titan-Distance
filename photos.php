...
<div class="container">
    ...
    foreach ($seasons as $s) {
        ...
        if (mysqli_num_rows($result) > 0) {
            echo "<h2 id='" . $s . "' class='border-bottom mb-2'>" . $s . "</h2>";
            ...
            while ($row = mysqli_fetch_array($result)) {
                ...
                echo "<div class='badge text-bg-primary position-absolute' style='top: 0.5rem; right: 0.5rem'>" . date("F j, Y", strtotime($row['date'])) . "</div>";
                if (!empty($row['cover'])) {
