...
foreach ($locations as $location => $primary) {
    ...
    $result = mysqli_query($con,"SELECT * FROM meets WHERE Location = '".addslashes($location)."' ORDER BY Date DESC");   
        $locationmeets = [];
        while($row = mysqli_fetch_array($result)) { 
            array_push($locationmeets,"meet = ".$row['id']);
        }
        echo "
                                    <div class=\"table-responsive\">
                                        <table class=\"table table-condensed table-sm table-hover\">";
if ($verified[$location] != 1) {
    echo "<caption>Only times imported into our database will be shown in this table. Please allow for some time for this to be updated.</caption>";
}
                                        echo"<thead>
                                                <tr>
                                                    <th>Time</th>
                                            ...
                                            <tbody>";
                                                $duplicates = [];
                                                $num = 0;
                        $result2 = mysqli_query($con,"SELECT * FROM overallxc"." WHERE (".join($locationmeets," OR ").") AND distance = '".$primary."' AND (school = 'Glenbrook South' OR school = 'Glenbrook South*') ORDER BY time ASC LIMIT 50");
...
?>
        ...
        <div class="my-3 d-flex justify-content-center">
            <a class="btn btn-primary"
                href="https://docs.google.com/spreadsheets/d/1ZtjRJh9UTv_eRSrQZr_6sS_SXlX5A9iftSoQsZiwFp0/edit#gid=573011141"
                role="button" target="_blank">Complete Listing</a>
        </div>
